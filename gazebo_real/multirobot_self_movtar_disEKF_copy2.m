...
global N omega_max tarx_hat tary_hat i_store iself_count self_tolerance...
    id Sigma_hat timer_cnt pose_x_store pose_y_store ave_err ave_com T first_com
...
robot_theta=[(robot_itheta(3)-2*pi);robot_itheta;(robot_itheta(1)+2*pi)]; %virtual agent 0th:=agent N-2pi;virtual agent 7th:=agent 1st+ 2pi
...
idb=[N;id(:,1);1];
    ...
    if first_com==1
                   ...
                   for i=1:N
                             ...
                             % communication always happens at the first
                             % step
                            ...
                            first_com=0; % the following steps
        ...
        for i=1:N
                       ...
                       if ((ubdi>=proximity)||(i_store(i,2)-r<=robot_itheta(i))||(i_store(i,2)+r>=robot_itheta(i)))
                       %if ((ubdi==0)||(ubdi>=proximity))  
                            com_time(i)=1;
