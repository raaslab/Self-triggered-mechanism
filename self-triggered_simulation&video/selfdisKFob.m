...
for i=1:N
    ...
    %itheta(i,2)=itheta(i,1)+1/4*T*(theta(i+2,1)-2*theta(i+1,1)+theta(i,1));% at the initial step, agent knows the exact info. of its meihhbors.
   %select the smallest condition number to obtain the optimal observation
                     idb=[i;1];
                     idpx=[px(i,1);px(1,1)];
                     idpy=[py(i,1);py(1,1)];
                     condi1=condobserva(tarx_hat(i,1),tary_hat(i,1),px(i,1),py(i,1),px(1,1),py(1,1));
                for j=2:N
                     condij=condobserva(tarx_hat(i,1),tary_hat(i,1),px(i,1),py(i,1),px(j,1),py(j,1));
                     if condij<condi1
                         condi1=condij;
                         idb=[i;j];
                         idpx=[px(i,1);px(j,1)];
    ...
    4, 20, Sigma_hat((2*i-1):2*i,1:2), idpx, idpy, idb, 1);
...
%tarx_hat(1,k), tary_hat(1,k), Sigma_hat(1:2,(2*k-1): 2*k), px(:,k), py(:,k), k);
    ...
    for i=1:N
          ...
          %select the smallest condition number to obtain the optimal observation
                     ...
                     idpx=[px(i,k);px(1,k)];
                     idpy=[py(i,k);py(1,k)];
                     condi1=condobserva(tarx_hat(i,k),tary_hat(i,k),px(i,k),py(i,k),px(1,k),py(1,k));
                for j=2:N
                     condij=condobserva(tarx_hat(i,k),tary_hat(i,k),px(i,k),py(i,k),px(j,k),py(j,k));
                     if condij<condi1
                         condi1=condij;
                         idb=[i;j];
                         idpx=[px(i,k);px(j,k)];
             ...
             [tarx_hat(i,k+1), tary_hat(i,k+1), Sigma_hat((2*i-1):2*i, (2*(k+1)-1):(2*(k+1))), tarxtrue(1,k), tarytrue(1,k)] = KF (...
              tarx_hat(i,k), tary_hat(i,k), Sigma_hat((2*i-1):2*i,(2*k-1): 2*k), idpx, idpy, idb, k);
