...
[tarx_hat(i,2), tary_hat(i,2), Sigma_hat((2*i-1):2*i, (2*2-1):2*2), tarxtrue(1,1), tarytrue(1,1)]=KF (...
    4, 20, Sigma_hat((2*i-1):2*i,1:2), head_px_tail(i:(i+2),1), head_py_tail(i:(i+2),1), 1);
end



R=zeros(2*N, max_step); %information agent keeps for its neighbors
pxstore=zeros(3*N, max_step);%information of x agent keeps for its neighbors
pystore=zeros(3*N, max_step);%information of y agent keeps for its neighbors
for i=1:N
    ...
    %/********/target input-----------
    [px(i,2),py(i,2)]=angulartopositionfun(itheta(i,2),tarx_hat(i,2),tary_hat(i,2));
     %itheta(i,2)=itheta(i,1)+1/4*T*(theta(i+2,1)-2*theta(i+1,1)+theta(i,1));% at the initial step, agent knows the exact info. of its meihhbors.
...
tarx_hat(i,k), tary_hat(i,k), Sigma_hat((2*i-1):2*i,(2*k-1): 2*k), pxstore((3*i-2):3*i,k+1), pystore((3*i-2):3*i,k+1), k);
            ...
            [px(i,k+1),py(i,k+1)]=angulartopositionfun(itheta(i,k+1),tarx_hat(i,k+1), tary_hat(i,k+1));
          else
...
tarx_hat(i,k), tary_hat(i,k), Sigma_hat((2*i-1):2*i,(2*k-1): 2*k), pxstore((3*i-2):3*i,k+1), pystore((3*i-2):3*i,k+1), k);
             ...
             count(i)=count(i)+1;
                 ...
                 itheta(i,k+1)=itheta(i,k)+T*omega_max*errp/abserrp;
                  [px(i,k+1),py(i,k+1)]=angulartopositionfun(itheta(i,k+1),tarx_hat(1,k+1), tary_hat(1,k+1));
                 elseif (abserrp<=ubdi)
